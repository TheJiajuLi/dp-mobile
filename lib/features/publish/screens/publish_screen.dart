...
''';
  ...
  // 从链接导入——真实接口 POST /auth/import/url（实测确认已上线：
  // axios+cheerio 抓取知乎/微信公众号/通用网页，返回 title/summary/
  // blocks/cover_image/source_url/platform/block_count，不是占位）。
  // 后端返回的 block 里引用块 type 是 'quote'，跟 EditorBlock 的
  // BlockType 枚举没有这个值对不上——EditorBlock.fromJson 遇到不认识的
  // type 会安全地退化成纯文字段落（orElse: () => BlockType.text），
  // 只是丢了引用块的样式，不会崩，导入内容还在
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      GestureDetector(
                        onTap: () => setSheetState(urlCtrl.clear),
                        child: Icon(
                          Icons.close,
                          size: 16,
                          color: Colors.grey[400],
                ...
                Row(
                  children: [
                    _PlatformBadge(
                      label: l10n.importPlatformZhihu,
                      initial: '知',
                    ...
                    _PlatformBadge(
                      label: l10n.importPlatformWechat,
                      initial: '公',
                    ...
                    const SizedBox(width: 6),
                    _PlatformBadge(
                      label: l10n.importPlatformGeneral,
                      isDark: isDark,
                    ),
                  ],
                ),
  ...
  Future<void> _doImport(
    ...
    if (url.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(l10n.importEmptyUrlError)));
      return;
    }

    setSheetState(() => setLoading(true));

    try {
      final res = await ref
          .read(apiClientProvider)
          .post('/auth/import/url', data: {'url': url});
      ...
      if (!res.success || res.data == null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(res.message ?? l10n.importFailedDefault),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(l10n.importFailedNetwork),
          backgroundColor: const Color(0xFFEF4444),
        ),
      );
    }
...
// 导入 Sheet 里的"支持平台"提示标签——知乎/公众号带彩色圆点+首字，
// 通用网页没有品牌色，用一个中性的 language 图标代替
    ...
    child: Row(
      ...
      children: [
        ...
        Text(
          ...
          style: TextStyle(
            fontSize: 11,
            color: isDark ? const Color(0xFF7A80A0) : const Color(0xFF555555),
          ),
        ),
      ],
    ),
